...
__global__ void BisectionMethodCUDA() {
	...
	// Ќаходим уникальный индекс
	unsigned int indx = blockIdx.x * blockDim.x + threadIdx.x;

	// Ќаходим рабочий интервал конкретного thread.
	// ¬се индексы по своей природе - положительные числа. ќднако, интервал должен учитывать возможные отрицательные корни.
	// ѕоэтому из каждого индекса вычитаем смещение, которое равно половине от общего числа thread.
	...
	do {
		...
		// ≈сли середина - корень уровнени€, то выводим результат на экран
	...
	// ≈сли привышено максимально допустимое количество итераций, то считаем, что корн€ на данном интервале не существует
	return;
}